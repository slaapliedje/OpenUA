...
Chunky → planar conversion for 68k bitplane machines (Atari ST/STE/TT, Amiga
OCS/ECS/AGA), in portable C, with host-side tests that check every routine
against a naive per-pixel reference.

Extracted from a Macintosh-to-68k game-engine port. The code is small; the
writeup below — what each optimisation was actually worth, and the three walls
that cap the whole approach — is the part that took months to learn.

## What's here

    include/c2p32.h      the 32-pixel bit transpose. NO dependencies.
    include/c2p4st.h     Atari ST/TT scatter: 4 planes, WORD-INTERLEAVED,
                         with a LUT remap and a flat-run fast path
    include/c2p_amiga.h  Amiga scatter: n separate planes, whole-surface
    src/c2p_amiga.c        and sub-rectangle forms
    tests/               host-compiled equivalence tests vs a naive reference
    RELICENSING.md       ★ read before publishing — see Licence below

`c2p32.h` and `c2p4st.h` are header-only. `c2p_amiga.c` is the only thing to
compile. There is no build system to adopt: drop the files in and add
`include/` to your include path.

    make test    run the equivalence tests (needs python3 + a host cc)
    make lint    compile-only sanity on the sources a consumer builds

## Choosing your scatter

"Planar" is not one format, and picking wrong costs a day:

| machine | mode | layout |
|---|---|---|
| Atari ST/STE low | 320×200, 16 col | 4 planes **word-interleaved** — `c2p4st.h` |
| Atari TT low | 320×480, 256 col | 8 planes word-interleaved (same idea, wider) |
| Amiga OCS/ECS | 320×200, 32 col | 5 **separate** planes — `c2p_amiga` |
| Amiga AGA | 320×200, 256 col | 8 separate planes — `c2p_amiga` |
| Atari Falcon (VIDEL) | 16 bpp | **chunky — needs none of this** |
| Amiga RTG | chunky | needs none of this |

The bit transpose is shared; only the scatter differs. Note the Falcon is a
chunky Atari and the TT is a planar one — do not infer from the badge.

## On performance, honestly

This is portable C and it is not the fastest c2p in existence. Hand-scheduled
assembler — Kalms' routines on Amiga being the canonical example — will beat it
by a wide margin, and if you are chasing a frame budget on real hardware you
should use those.

What this gives you instead is: readable, tested, one transpose covering both
plane layouts, and 68000-clean. In our port the transpose stopped being the
bottleneck well before its inner loop did — which is the actual subject of the
writeup below.

## Licence

**MIT.** Extracted from a GPL-2.0 project by that project's sole author, who
relicensed it — see [PROVENANCE.md](PROVENANCE.md) for how that was verified
and how this copy relates to the upstream one.

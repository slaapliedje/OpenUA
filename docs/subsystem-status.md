...
| **Audio / music / sound** (.slb engine) | 6/6 | ✅ | `audio-wall.md` — LIVE on every target and ear-verified: Falcon CODEC, TT STE-DMA, ST/STE, Amiga **AHI-preferred with a Paula fallback** (ADR-0021, confirmed on a real A1200 2026-09-05). Four-tone synth music from `MUSIC.SLB`, now resolved **design-first** (a module's own soundtrack plays; ADR-0011 applied to music, 2026-09-05). The bank is **re-resolved when the design changes** (`port_bank_recheck` at every song start, 2026-09-07) — it used to load once at boot, so a module picked from the menu played the base game's music. *(This row said "every output leaf stubbed → MUTED" until 2026-09-05 — that had been false for weeks; milestone.md §3 even cited it as a stale-table hazard while this register still carried it.)* |
| **GLIB palette + colour cycling** (jt993 / jt1069 / jt1066 / jt1067, `l6e58`) | 5/5 | ✅ | `glib-palette-subsystem.md`. Faithful lifts, with **three port rules on top** (ADR-0022, 2026-09): the port's DIRECT band installs free the cycle ranges they overlap, as the allocator's Phase 3a does (a picture's cycle used to outlive it and animate the walk's floor, #21); `l6e58` loads CLUT **0..15** too (the Mac reserved them; DOS art paints with them, #23); and the opaque compressed blit keeps **index 255** as a colour (#24). `-DFRUA_CYCTRACE` logs to DBG.LOG; AGA has `-DFRUA_PALTEST` for a one-photo colour-path check on real hardware. On AGA a palette write **rides with the next present** (`dsp_backend_t.palette_with_present`, TODO 25; `video.cfg agapalsync=off` opts out) so an outgoing picture is never shown in the incoming palette. |

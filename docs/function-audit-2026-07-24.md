...
| Training's **guild class mask** | `jt557` L7324 gate | `guildMask = g_a5_28006[48]` is **always 0**, so `(haveMask & guildMask) == 0` always holds and every character gets "we don't train that class here" rather than the correct verdict (for a level-6 fighter on 50,000 XP it should be "Not Enough Experience"). What the Mac does with that byte, from the disassembly: the **event** path sets it from the Training-Hall event's class mask (`CODE 20 @0x2dc8`, `gameRec[48] = ev[8]`); both **trainer-UI** paths save it, force `0xFF`, call `jt557` in a `while (-22731 == 0)` loop and restore it (`CODE 17 @0x2840`, `CODE 10 @0x5cbe`); and the **CODE 12 hall arm we implement calls `jt557` bare**, so a menu-entered hall must get the byte from somewhere earlier that the port never fills. Ruled out: `-28006` is not the GAME001 record — at runtime its byte 18 is 4 and byte 48 is 0, where HEIRS' GAME001.DAT has 103 and 5. Finding the real writer is the remaining work; do not force `0xFF` at our call site, because that is the trainer UI's behaviour and not this arm's. |

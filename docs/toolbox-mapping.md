...
| Event Manager       | `GetNextEvent`, mouse/keyboard   | `EventRecord`, the masks / modifiers, a 16-slot posted FIFO, `TickCount` over `_hz_200`, `GetNextEvent` / `EventAvail` / `PostEvent` / `FlushEvents` / `WaitNextEvent`, keyboard via BIOS `Bconstat` / `Bconin` + `Kbshift`, and updateEvt synthesis from any window's non-empty `updateRgn` (`compat/events.c`); mouse, `activateEvt`, and `autoKey` follow with the IKBD-packet driver | wip |

...
|---------------------|----------------------------------|-------------------------------------------------------------|---------|
| QuickDraw           | All drawing, `CopyBits`, regions | Geometry core, the `GrafPort`, and `GetPort`/`SetPort` in `compat/quickdraw.c`; drawing awaits the display HAL | wip |
...
| Window Manager      | Editor & play-UI windows         | Reimplemented in the shim — window record, list, lifecycle (`compat/windows.c`); frames & colour windows await the HAL | wip |

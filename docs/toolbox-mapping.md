...
|---------------------|----------------------------------|-------------------------------------------------------------|---------|
| QuickDraw           | All drawing, `CopyBits`, regions | Geometry core, the `GrafPort` / `CGrafPort` and Color QuickDraw types, `NewPixMap`, rectangular regions, `GetPort`/`SetPort`, the screen port over the display HAL back buffer (`qd_attach_screen`), the rect primitives (`EraseRect`, `PaintRect`, `FrameRect`), the line family (`MoveTo`, `LineTo`, `GetPen`), the ovals (`PaintOval`, `FrameOval`), and `CopyBits` (8-bit same-size `srcCopy`) in `compat/quickdraw.c`; pen state, patterns, text, scaling, and the other transfer modes still to come | wip |

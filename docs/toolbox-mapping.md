...
| Event Manager       | `GetNextEvent`, mouse/keyboard   | `EventRecord`, the masks / modifiers, a 16-slot posted FIFO, `TickCount` over `_hz_200`, `GetNextEvent` / `EventAvail` / `PostEvent` / `FlushEvents` / `WaitNextEvent`, keyboard via BIOS `Bconstat` / `Bconin` + `Kbshift`, mouse position / button via the IKBD-packet handler (a Supexec-installed `mousevec` trampoline in `platform/input.c` decoding the 3-byte packets into `g_mouse_x` / `g_mouse_y` / `g_mouse_btn`), and updateEvt synthesis from any window's non-empty `updateRgn` (`compat/events.c`); `activateEvt`, `autoKey`, and `mouseDown` / `mouseUp` synthesis follow | wip |

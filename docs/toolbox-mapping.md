...
| Menu / Dialog       | Editor & tools UI                | Reimplemented in the shim; Mac-style widgets drawn into the HAL surface (ADR-0006). `Alert(id, filter)` is up: parses ALRT + DITL from the resource fork, opens a modal window, paints button frames + static text into the screen port, spins a small event loop until keypress / click dismisses. Menu Manager + the full `GetNewDialog` / `ModalDialog` machinery still to come. | wip |

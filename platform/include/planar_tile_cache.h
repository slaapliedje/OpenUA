...
typedef struct {
	...
	short          w, h;
	unsigned long  epoch;             /* remap generation it was built for */
...
static unsigned short *pt_cache_reserve(pt_cache_t *c, short slot, short idx,
                                        short w, short h, unsigned long epoch)
{
	...
	for (i = 0; i < c->n_ent; i++) {
		if (c->ent[i].slot == slot && c->ent[i].idx == idx) {
			if (c->ent[i].w == w && c->ent[i].h == h) {
				c->ent[i].epoch = epoch;   /* rebuild in place */
				c->rebuilt++;
				return c->pool + c->ent[i].off;
			}

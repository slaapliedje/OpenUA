...
case "$cmd" in
	build)
		cd "$REPO"; exec make "$@"
		;;
	start|shot|shots|dump|key|click|drag|beginplay|wait|dbg|log|quit)
		ensure_display
		exec "$UI" "$cmd" "$@"
		;;
	stop)
	...
	sound)
		...
		;;
	*)
		echo "usage: $(basename "$0") build | start | shot <png> | shots <png> | key <keysym>... | click <x> <y> [button] | drag <x1> <y1> <x2> <y2> [button] | beginplay | wait <regex> [n] | log | quit | stop | smoke <png> | sound <wav>" >&2
		exit 1
		;;
esac
